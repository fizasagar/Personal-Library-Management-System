...
if st.session_state.current_view == "add":
    st.markdown(
        "<h2 class='sub-header'> ➕ Add a New Book </h2>", unsafe_allow_html=True
    )
    ...
    if not st.session_state.library:
        st.markdown(
            "<div class='warning-message'> ⚠️ Your Library is empty. Add some books to get started! </div>",
            unsafe_allow_html=True,
        )
    else:
        cols = st.columns(2)
...
""",
                ...
                with col1:
                    if st.button(
                        f"Remove", key=f"remove_{i}", use_container_width=True
                    ):
                        if remove_book(i):
                            st.rerun()
                with col2:
                    ...
                    status_label = (
                        "Mark as Read" if not book["read_status"] else "Mark as Unread"
                    )
                    if st.button(
                        status_label, key=f"status_{i}", use_container_width=True
                    ):
    ...
    if st.session_state.book_removed:
        st.markdown(
            "<div class='success-message'> 🗑️ Book removed successfully!</div>",
            unsafe_allow_html=True,
        )
        st.session_state.book_removed = False
...
if hasattr(st.session_state, "search_results"):
    if st.session_state.search_results:
        ...
        st.markdown(
            "<div class='warning-message'>⚠️ No books found matching your search.</div>",
            unsafe_allow_html=True,
        )

elif st.session_state.current_view == "stats":
    st.markdown(
        "<h2 class='sub-header'>📊 Library Statistics</h2>", unsafe_allow_html=True
    )

    if not st.session_state.library:
        st.markdown(
            "<div class='warning-message'>⚠️ Your library is empty. Add some books to see stats!</div>",
            unsafe_allow_html=True,
        )
    else:
        ...
        if stats["authors"]:
            st.markdown(
                "<h3 class='top-authors'>🏆 Top Authors</h3>", unsafe_allow_html=True
            )
